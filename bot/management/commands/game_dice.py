...
def apply_random_effect(character):
    effects = {
        ...
        "Increase health by 20%": lambda: setattr(
            character, "hp", int(character.hp * 1.2)
        ),
        "Disable dodge": lambda: setattr(
            character, "dodge_modifier", -character.calculate_dodge()
        ),
        "Damage reduction by 10%": lambda: setattr(
            character, "dmg", int(character.dmg * 0.9)
        ),
        "Increase damage by 15%": lambda: setattr(
            character, "dmg", int(character.dmg * 1.15)
        ),
        "Reduce damage by 15%": lambda: setattr(
            character, "dmg", int(character.dmg * 0.85)
        ),
        "Increase dodge by 20%": lambda: setattr(
            character, "dodge_modifier", character.dodge_modifier + 20
        ),
        "Reduce dodge by 20%": lambda: setattr(
            character, "dodge_modifier", max(0, character.dodge_modifier - 20)
        ),
        "Increase critical hit chance by 10%": lambda: setattr(
            character, "crit_chance_modifier", character.crit_chance_modifier + 0.1
        ),
        "Reduce critical hit chance by 10%": lambda: setattr(
            character,
            "crit_chance_modifier",
            max(0, character.crit_chance_modifier - 0.1),
        ),
        "Regenerate 5% health each turn": lambda: setattr(
            character, "regenerate", True
        ),
...
def reset_effects(character, original_methods):
    effects_reset = {
        ...
        "Increase CP by 10%": lambda: setattr(
            character, "cp", int(character.cp // 1.1)
        ),
        "Reduce CP by 10%": lambda: setattr(character, "cp", int(character.cp // 0.9)),
    }
