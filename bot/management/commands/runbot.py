...
def clear_and_create_enemies():
    ...
    boss_bear = [BossBear(name="Furious Bear", level=1, skills={"root": True})]

    boss_fairy = [BossFairy(name="Ancient Fairy", level=1, skills={"rage": True})]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def send_welcome(message):
            ...
            try:
                ...
                if created:
                    bot.reply_to(
                        message,
                        "Welcome to the game! Please enter your character's name:",
                    )
                else:
        ...
        @bot.message_handler(
            func=lambda message: get_or_none(Character, user_id=message.from_user.id)
                                 and get_or_none(Character, user_id=message.from_user.id).state
                                 == "awaiting_nickname"
        )
            ...
            if user:
                ...
                if Character.objects.filter(nickname=nickname).exists():
                    bot.reply_to(
                        ...
                        re.match(ALLOWED_CHARACTERS_PATTERN, nickname)
                        and len(nickname) <= 20
                ):
                    ...
                    bot.reply_to(
                        message, "Great! Now choose your class:", reply_markup=markup
                    )
                else:
                    bot.reply_to(
                        message,
                ...
                bot.reply_to(
                    message, "Character not found. Please start a new game with /start."
                )

        @bot.message_handler(
            func=lambda message: Command.get_or_none(
                Character, user_id=message.from_user.id
            )
                                 and Command.get_or_none(Character, user_id=message.from_user.id).state
                                 == "class_selection"
        )
        def set_class(message):
            ...
            if user:
                ...
                if role in class_descriptions:
                    ...
                    back_button = telebot.types.KeyboardButton(
                    ...
                    bot.reply_to(
                        message,
                        f"{description} Choose your action:",
                        reply_markup=markup,
                    )
                else:
        ...
        def show_location_selection(message):
            ...
            catacombs_button = telebot.types.KeyboardButton(
                "Ancient Catacombs (Normal)"
            )
            ...
            bot.send_message(
                message.chat.id, "Choose your location:", reply_markup=markup
            )
        ...
        @bot.message_handler(
            func=lambda message: message.text == "Mistery Forest (Easy)"
        )
        @requires_character
        def handle_forest(message):
            ...
            user.state = "location_1_level_{}".format(current_level)  # Устанавливаем начальное состояние
            user.save()

            markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
            for i in range(current_level, 11):
                markup.add(telebot.types.KeyboardButton("Mistery Forest Level {}".format(i)))
        ...
        @bot.message_handler(
            func=lambda message: message.text.startswith("Mistery Forest Level")
        )
        @requires_character
        def handle_forest_level(message):
            user = Character.objects.get(user_id=message.from_user.id)
            expected_state = "location_1_level_{}".format(user.current_forest_level)

            # Отладочное сообщение
            logging.info("User state before handling forest level: %s", user.state)
            # Отладочное сообщение
            ...
            if enemies:
                user.state = "location_1_level_{}".format(level)  # Устанавливаем новый state для текущего уровня
                user.in_battle = True

                # Отладочное сообщение
                logging.info("User state after setting new level: %s", user.state)
        ...
        @bot.message_handler(
            func=lambda message: message.text == "Ancient Catacombs (Normal)"
        )
        @requires_character
        def handle_catacombs(message):
            ...
            user.state = "location_2_level_{}".format(current_level)  # Устанавливаем начальное состояние
            user.save()

            # Отладочное сообщение
            logging.info("User state after choosing Ancient Catacombs: %s", user.state)

            markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
            for i in range(current_level, 11):
                markup.add(telebot.types.KeyboardButton("Ancient Catacombs Level {}".format(i)))
            markup.add(telebot.types.KeyboardButton("Back"))
            bot.send_message(message.chat.id, "Choose your level:", reply_markup=markup)

        @bot.message_handler(
            func=lambda message: message.text.startswith("Ancient Catacombs Level")
        )
        @requires_character
        def handle_catacombs_level(message):
            user = Character.objects.get(user_id=message.from_user.id)
            expected_state = "location_2_level_{}".format(user.current_catacombs_level)

            # Отладочное сообщение
            logging.info("User state before handling Ancient Catacombs level: %s", user.state)
            # Отладочное сообщение
            logging.info("Expected state: %s", expected_state)
            ...
            if enemies:
                user.state = "location_2_level_{}".format(level)  # Устанавливаем новый state для текущего уровня
                user.in_battle = True

                logging.debug("User state after setting new Ancient Catacombs level: %s", user.state)
        ...
        def get_catacombs_level_enemies(level):
            if level == 1:
                ...
                return [BossBear(name="Furious Bear", level=1, skills={"rage": True})]
            return None
        ...
        def handle_magma(message):
            ...
            if user.state != "location_selection":
                bot.reply_to(message, "You are not ready for this location yet.")
                return
            # Текущий уровень магмовых полей пользователя
            current_level = user.current_magma_level
            # Устанавливаем начальное состояние
            user.state = "location_3_level_{}".format(current_level)
            user.save()

            # Отладочное сообщение
            logging.info("User state after choosing Magma Fields: %s", user.state)

            markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
            for i in range(current_level, 11):
                markup.add(telebot.types.KeyboardButton("Magma Fields Level {}".format(i)))
            markup.add(telebot.types.KeyboardButton("Back"))
            bot.send_message(message.chat.id, "Choose your level:", reply_markup=markup)

        @bot.message_handler(
            func=lambda message: message.text.startswith("Magma Fields Level")
        )
        @requires_character
        def handle_magma_level(message):
            user = Character.objects.get(user_id=message.from_user.id)
            expected_state = "location_3_level_{}".format(user.current_magma_level)

            # Отладочное сообщение
            logging.info("User state before handling Magma Fields level: %s", user.state)
            # Отладочное сообщение
            logging.info("Expected state: %s", expected_state)
            ...
            if enemies:
                user.state = "location_3_level_{}".format(level)  # Устанавливаем новый state для текущего уровня
                user.in_battle = True

                # Отладочное сообщение
                logging.info("User state after setting new Magma Fields level: %s", user.state)
        ...
        def get_magma_level_enemies(level):
            if level == 1:
                ...
                return [
                    Golem(name="Stone Golem", level=1),
                    Cerberus(name="Cerberus", level=2),
                ]
            elif level == 3:
                return [
                    Cerberus(name="Cerberus", level=2),
                ...
                return [
                    BossPhoenix(name="Phoenix", level=1, skills={"reincarnation": True})
                ]  # Example boss name and skills, adjust as needed
            return None
        ...
        def handle_confirmation(message):
            if message.text.lower() == "yes":
                try:
                    ...
                    bot.send_message(
                        message.chat.id,
                        "Your progress has been reset. You can now start anew./start",
                    )
